...
import { BarChart3, FileText, MessageSquare, ArrowLeft, RefreshCw, Sparkles, Languages, HelpCircle, FileCheck, Landmark, Tag, CheckCircle2, XCircle, Award, RotateCcw, Check } from 'lucide-react';
...
const parseQuizText = (rawText: string): QuizQuestion[] => {
  ...
  blocks.forEach((block, idx) => {
    ...
    lines.forEach(line => {
      if (line.match(/^(\d+:|\d+\.|\?)/) || (!qText && !line.startsWith("-") && !line.startsWith("**"))) {
        const cleaned = line.replace(/^(\d+:|\d+\.|\?)/, '').replace(/^Question \d+:/i, '').trim();
        if (cleaned) qText = cleaned;
      } else if (line.match(/^-\s*\[\s*\]\s*([A-D])\)\s*(.*)/i) || line.match(/^-?\s*([A-D])\)\s*(.*)/i)) {
        const match = line.match(/^-\s*\[\s*\]\s*([A-D])\)\s*(.*)/i) || line.match(/^-?\s*([A-D])\)\s*(.*)/i);
        if (match) {
...
export const DocumentAnalysis: React.FC = () => {
  ...
  const triggerTool = async () => {
    ...
    try {
      ...
      let url = `/api/documents/${id}/stream-tool?tool=${selectedTool}`;
      if (selectedTool === 'translate' && translationLanguage) {
        url += `&language=${encodeURIComponent(translationLanguage)}`;
      }
  ...
  const handleTabChange = (category: 'overview' | 'insights' | 'entities' | 'faq' | 'tools') => {
    ...
    if (category === 'overview' || category === 'tools') {
      setActiveOutput('');
      return;
    }
  ...
  const tools = [
    ...
    { id: 'simplify', name: 'Simplify Language' },
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8 relative">
      ...
      <div className="mb-6">
        ...
        <p className="text-slate-400 text-xs mt-1">Audit statistics, metadata properties, entities, FAQs, and trigger interactive quiz & tool drafts.</p>
      </div>
              ...
              <div className="space-y-6">
                <div className="flex flex-col sm:flex-row gap-4 items-end bg-white/5 p-4 rounded-xl border border-white/5">
                  <div className="flex-grow">
                    <label className="block text-[10px] text-slate-500 uppercase tracking-widest font-bold mb-2">Select AI Draft Tool</label>
                    <select
                      value={selectedTool}
                  ...
                  <button
                    onClick={triggerTool}
                    disabled={streaming}
                    className="w-full sm:w-auto px-5 py-2.5 rounded-lg bg-indigo-500 hover:bg-indigo-600 text-white text-xs font-semibold shadow-md transition-all flex items-center justify-center gap-2 disabled:opacity-50"
                  >
                    {streaming ? 'Drafting...' : 'Run Tool'}
                  </button>
                </div>
                ...
                <div className="prose prose-invert max-w-none text-sm leading-relaxed text-slate-300 bg-white/5 p-5 rounded-xl border border-white/5 min-h-[300px]">
                  {activeOutput ? (
                    ...
                    <div className="text-center py-16 text-slate-500">
                      ...
                      <p className="font-semibold text-xs">Run a tool above to generate AI content dynamically.</p>
                    </div>
                  )}
                </div>
              </div>
